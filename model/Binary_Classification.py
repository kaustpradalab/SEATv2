""" most of our train code is in this files func:train_ours.
if you want to get the data used to plotting,please replace this file with Binary_Classification_preturb.py and modify commented code in it. """
import numpy as np
import json
import os
import shutil
...
class Model():
    def __init__(self, configuration, args, pre_embed=None):
        ...
        self.lambda_1 = args.lambda_2
        self.lambda_2 = args.lambda_1
    ...
    def eval_sens(self, dataset, X_PGDer=None,args=None,train=False):
        """ get Sens score in eq.10 in our paper (A1.0.5) """
        self.encoder.train()
        self.decoder.train()
    ...
    def eval_comp_suff(self, dataset, X_PGDer=None,args=None,train=False):
        """ get comp&suff score in eq8&9 in our paper (A1.0.4) """
        self.encoder.eval()
        self.decoder.eval()
    ...
    #get the baseline JSD/TVD score (vanilla attention model's score)
    ...
    def train_ours(self,dataset,
        ...
        """main code used to train and evaluate ours model ."""
        self.encoder.train()
        self.decoder.train()
        ...
        for n in tqdm(batches):
            ...
            old_att = att_sta[n:n+bsize]
            old_att=torch.Tensor(old_att).to(device)
            #When it's test mode,preturb_x will be true. 
            if preturb_x:
                ...
                new_att = batch_data.attn
                px_l1_att_diff.update(torch.abs(old_attn - new_att).mean().item(), len(batch_doc))
                px_l2_att_diff.update(torch.pow(old_attn - new_att, 2).mean().item(), len(batch_doc))
                new_pred = torch.sigmoid(batch_data.predict)
            ...
            if self.args.method == "ours":
                ...
                self.encoder(batch_data,revise_embedding=new_embedd)
                self.decoder(data=batch_data)
                #get Stability-of-Prediction loss.(with adv prefix)(in our paper Definition 2)
                new_out_adv = torch.sigmoid(batch_data.predict)
                new_att_adv = batch_data.attn
                #get Similarity-of-Explanation loss.(with sim prefix)
                sim_topk_loss = topk_overlap_loss(new_att_adv,
                                                new_att,K=self.K, metric=self.topk_prox_metric).mean()
                ...
                self.encoder(batch_data,revise_embedding=new_embedd_sta)
                self.decoder(data=batch_data)
                #get Stability-of-Explanation loss.(with sta prefix)
                new_att_sta = batch_data.attn
                sta_topk_loss = topk_overlap_loss(new_att_sta,
                            new_att,K=self.K, metric=self.topk_prox_metric).mean()
            ...
            if self.args.method == "ours":
                sim_topk_loss_mean += float(sim_topk_loss.data.cpu().mean().item())
                sta_topk_loss_mean += float(sta_topk_loss.data.cpu().mean().item())
    ...
    def train(self,
              ...
              PDGer=None,
              args=None):
        '''this function is used to train vanilla model.'''
        if train:
            self.encoder.train()
            self.decoder.train()
        else:
