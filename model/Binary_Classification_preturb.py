...
from attention.utlis.utlis import topk_overlap_loss,topK_overlap_true_loss,jaccard_similarity
    ...
    def train_ours(self,dataset,
        ...
        for n in tqdm(batches):
            ...
            if preturb_x:
                ...
                for value in att_values:
                    max_att_list.append(value.item())
                jsd_values = js_divergence(old_attn, new_att).squeeze(1).cpu().data.numpy()
                for value in jsd_values:
                    jsd_list.append(value.item())
                label_values = batch_data.label.cpu().data.numpy()
                for value in label_values:
            ...
            if self.args.method == "ours":
                ...
                max_att_list_cpu = max_att_list
                jsd_list_cpu = jsd_list
                jsd_list_cpu=np.array(jsd_list_cpu)
                max_att_list_cpu=np.array(max_att_list_cpu)
                label_list_cpu=np.array(label_list)
                with open('./plot_violin_data/ours_jsd_list_'+self.args.dataset+'.pkl', 'wb') as file:
                    pickle.dump(jsd_list_cpu, file)
                with open('./plot_violin_data/ours_max_att_list_'+self.args.dataset+'.pkl', 'wb') as file:
                    pickle.dump(max_att_list_cpu, file)
                with open('./plot_violin_data/ours_label_list_'+self.args.dataset+'.pkl', 'wb') as file:
                    pickle.dump(label_list_cpu, file)
            batch_data.to_cpu()
            del batch_data
