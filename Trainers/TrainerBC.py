...
class Trainer():
    ...
    def train_ours(self, dataset, args=None):
        ...
        for i in tqdm(range(args.n_epoch)):
            ...
            if br:
                comp, suff=self.copy_model.eval_comp_suff(dataset['test'], X_PGDer=self.X_PGDer, args=args)
                sens=self.copy_model.eval_sens(dataset['test'], X_PGDer=self.X_PGDer, args=args)
                wandb.log({
                "suff_te": suff,
                "comp_te": comp,
                "sens_te": sens,
